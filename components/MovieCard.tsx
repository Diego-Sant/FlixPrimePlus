...
const MovieCard: React.FC<MovieCardProps> = ({data}) => {
    ...
    return (
        <div className="group col-span relative lg:h-[11vw] md:h-[15vw] h-[20vw] mb-5 xl:mb-36 lg:mb-20" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            ...
            <div className="opacity-0 absolute top-0 transition duration-200 z-30 w-[25vw] h-[20vw] sm:visible delay-300 scale-0 group-hover:scale-110 group-hover:-translate-y-[6.5vw] group-hover:translate-x-[2.7vw] group-hover:opacity-100">
                { data?.videoUrl ? 
                ...
                <div className="z-30 bg-zinc-800 p-2 lg:p-4 absolute w-full transition shadow-md rounded-b-md">
                    <div className="flex flex-row items-center gap-3">
                        <div onClick={() => router.push(`/assistir/${data?.id}`)} className="cursor-pointer w-6 h-6 lg:w-10 lg:h-10 bg-white rounded-full flex justify-center items-center transition hover:bg-neutral-300">
                            <BsFillPlayFill className="marginfifteen" size={30} />
                        </div>
                        <div className="favoritesmall">
                        <FavoriteButton movieId={data?.id} />
                        </div>
                        {data?.videoUrl ? <div onClick={handleToggleMute} className="cursor-pointer mutesmall w-6 h-6 lg:w-10 lg:h-10 bg-transparent border-white border-2 rounded-full flex justify-center items-center transition hover:bg-neutral-300">
                        ...
                        <div onClick={() => openModal(data?.id)} className="cursor-pointer ml-auto group/item w-6 h-6 lg:w-10 lg:h-10 border-white border-2 rounded-full flex justify-center items-center transition hover:border-neutral-300">
                            <BsChevronDown className="text-white lg:text-[15px] text-[10px] group-hover/item:text-neutral-300" />
                        </div>
                    </div>
                    {data?.rating ? 
                    ...
                    <div className="flex flex-row mt-3 gap-2 items-center">
                        <div className="h-5 w-5">
                            {/* eslint-disable-next-line */}
                            {data?.contentRatingUrl ? <img src={data?.contentRatingUrl} className="smallcontent" alt="Classificação do Conteúdo"/> : ""}
                        </div>
                        {data?.duration ? <p className="text-white text-[10px] lg:text-sm durationsmall">{data?.duration}</p> : ""}
                        {/* eslint-disable-next-line */}
                    ...
                    <div className="text-white descriptiontext text-center text-[13px] lg:text-sm mt-3">
                        {formattedGenre}
                    </div>
                </div>
            </div>
