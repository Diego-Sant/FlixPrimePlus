...
const Billboard = () => {
    ...
    return (
        <>
            {data && (
                <div className="relative h-[56.25vw] -top-20"> {/* o elemento terá 56.25% da largura da tela */}
                    ...
                    <div className="buttonsize absolute top-[65%] z-20 md:top-[60%] xl:top-[80%] right-20">
                        <button className="absolute text-white border-2 z-10 right-16 rounded-full p-2" onClick={handleToggleMute}>
                            {muted ? <FaVolumeMute /> : <FaVolumeUp />}
                        </button>
                        {/* eslint-disable-next-line */}
                        <img className="h-10 w-10 mt-2.5" src={data?.contentRatingUrl} alt={"Classificação do Conteúdo"}/>
                    </div>
                    <div className="absolute top-[55%] md:top-[60%] xl:top-[65%] ml-4 z-10 md:ml-16">
                        ...
                        <div className="flex flex-row items-center mt-3 md:mt-4 gap-3">
                            <button className="bg-white text-white bg-opacity-40 rounded-md py-1 md:py-2 px-2 md:px-4 w-auto text-xs lg:text-lg font-semibold flex cursor-pointer flex-row items-center hover:bg-opacity-20 transition"><AiOutlineInfoCircle className="mr-1" /> Mais informações</button>
                        </div>
                    </div>
